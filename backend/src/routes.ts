...
export function getRouter(app:IRouter) {
  ...
  app.get("/packages/:id",[authJwt.verifyToken,authJwt.isUser,authJwt.isAdmin], renderPackeges);
  app.post("/newPackages",[authJwt.verifyToken,authJwt.isUser,authJwt.isAdmin], addPackage);
  app.get("/package/:id",[authJwt.verifyToken,authJwt.isUser,authJwt.isAdmin], stornoParckage);
  app.get("/createLabel/:id",[authJwt.verifyToken,authJwt.isUser,authJwt.isAdmin], generateLabel)
